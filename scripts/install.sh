...
main() {
  if [ -f "$ENV_FILE" ]; then
    ...
    if [ -z "${HOME_MANAGER_DIR:-}" ] || [ ! -d "$HOME_MANAGER_DIR" ]; then
      error "Env file points to a non-existent directory: '$HOME_MANAGER_DIR'. Please remove '$ENV_FILE' to start over."
    fi

    local bootstrap_script_path="$HOME_MANAGER_DIR/$BOOTSTRAP_SCRIPT"
    if [ ! -f "$bootstrap_script_path" ]; then
      error "Bootstrap script not found in '$HOME_MANAGER_DIR'. Installation may be corrupt. Please remove '$ENV_FILE' and run again."
    fi
      ...
      case "$choice" in
        [Rr])
          if [ "$bootstrap_exists" = true ]; then
            ...
            info "Saving installation path to '$ENV_FILE'..."
            mkdir -p "$(dirname "$ENV_FILE")"
            echo "HOME_MANAGER_DIR=\"$dest_dir\"" > "$ENV_FILE"
